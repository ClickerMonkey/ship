...
/**
 * A tetris field is a grid of Squares that was built up by placing blocks on 
 * the field. As the blocks build up on the field if there are full rows, or 
 * 'lines', these can be removed from the field for more building of blocks.
 * 
...
public class TetrisField 
{
	...
	// The 2-dimensional array of squares which make up this tetris field.
	private Square[][] grid;
	...
	/**
	 * Clears the square grid so this field contains no squares.
	 */
	public void clear()
	{
		for (int y = 0; y < HEIGHT; y++)
	...
	/**
	 * Determines if a given point exists on this field and also is not 
	 * already a square.
	 * 
	...
	/**
	 * This is used for the game over screen where all rows starting from the 
	 * bottom fill up. This will fill a row's empty spaces with random squares.
	 * 
	...
	public void fillInSquares(int row)
	{
		for (int x = 0; x < WIDTH; x++)
			if (grid[row][x] == null)
				grid[row][x] = new Square(x, row, null); // null => Random
	}
	...
	/**
	 * This will check the tetris field for any complete lines. For every line 
	 * thats found the blocks above that line are shifted downwards. This will 
	 * return how many, if any, were removed from this field.
	 * 
	...
	/**
	 * This will shift down a square based on the given x value (column) and 
	 * the current row to shift down onto.
	 * 
