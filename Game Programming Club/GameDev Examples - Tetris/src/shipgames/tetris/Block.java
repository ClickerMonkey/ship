...
/**
 * A block is a 'Tetromino' consisting of four squares
 * arranged in a specific order based on its BlockType.
 * The BlockTypes are all the possible combinatons of
 * putting four blocks where every block is connected
 * by the side to another block.
 * 
...
public class Block 
{
	...
	/**
	 * Initializes a Block based on the GameField
	 * containing it, its initial x and y position, and
	 * its block type.
	 * 
	...
	/**
	 * This initializes this block at a position
	 * setting its squares positions based on this blocks
	 * type. If this block's type is set to Random
	 * it is reset as one of the predefined blocks.
	 * 
	 * @param x => The x value, or column this Block is on.
	...
	/**
	 * This will create a block that is directly
	 * below this one and is resting on the gamefield.
	 * This is used so the player can see exactly how
	 * the Block is going to land on the field.
	 */
	public Block getShadow()
	{
		Block shadow = new Block(field, 0, 0, type);
		// Set the shadows current position
		shadow.next[0].set(squares[0].getLocation());
		shadow.next[1].set(squares[1].getLocation());
		shadow.next[2].set(squares[2].getLocation());
		shadow.next[3].set(squares[3].getLocation());
		// While there is no intersection, go down.
		while (!shadow.intersectsField())
		{
			shadow.next[0].y++; 
			shadow.next[1].y++;
			shadow.next[2].y++;
		...
		// Move back up one so there no intersection.
		shadow.next[0].y--; 
		shadow.next[1].y--;
		shadow.next[2].y--; 
		shadow.next[3].y--;
		// Set the shadows next positions then move to them.
		shadow.moveToNext();
	...
	/**
	 * This will return whether the next positions computed
	 * will intersect with any existing square on the field.
	 * 
	...
	/**
	 * This will reset the positions of the next points
	 * to the positions of the current blocks
	 */
	...
	/**
	 * This will move the squares of the block to
	 * the next points on the field.
	 */
	...
	/**
	 * This will return the center of this block 
	 * which is used for rotation and is the second block.
	 */
	...
	/**
	 * This will return a square from this block based
	...
	/**
	 * Gets the row of the bottom most square in this block.
	 */
	public int getBottom()
	{
		return Math.max(squares[0].getY(), 
			   Math.max(squares[1].getY(), 
			   Math.max(squares[2].getY(), squares[3].getY())));
	}
	
	/**
	 * Gets the column of the left most square in this block.
	 */
	public int getLeft()
	{
		return Math.min(squares[0].getX(), 
			   Math.min(squares[1].getX(), 
			   Math.min(squares[2].getX(), squares[3].getX())));
	}

	/**
	 * Gets the column of the right most square in this block.
	 */
	public int getRight()
	{
		return Math.max(squares[0].getX(), 
			   Math.max(squares[1].getX(), 
			   Math.max(squares[2].getX(), squares[3].getX())));
	}
	
	/**
	 * This will return the height of this block in squares.
	 */
	...
	/**
	 * This will return the width of this block in squares.
	 */
	public int getWidth()
	{
