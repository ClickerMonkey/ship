...
public class Texture {
	...
	/** The width of the texture */
	private float		texWidthInv;

	/** The height of the texture */
	private float		texHeightInv;
	...
	public void bind() {
		...
		if (currentID != textureID) {
			glBindTexture(target, currentID = textureID);	
		}
	}
	
	public Source getSource(int x, int y) {
		return new Source((float)x / texWidth, (float)y / texHeight);
	}
	
	public SourceTile getTile(int x, int y, int width, int height) {
		return new SourceTile(this, x * texWidthInv, y * texHeightInv, 
				(x + width) * texWidthInv, (y + height) * texHeightInv);
	}
	public SourceTile getTile() {
		return new SourceTile(this, 0f, 0f, 1f, 1f);
	}
	...
	public void setTextureHeight(int texHeight) {
		...
		this.texHeightInv = 1f / texHeight;
		setHeight();
	}
