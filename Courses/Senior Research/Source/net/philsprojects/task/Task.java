 ...
 * started by being cancelled, or by not completing by a specific time. A task
 * uses a state machine which can hold Intialized, Waiting, Running, and 
 * Finished states. Substates of Finished exist which are Success, TimedOut, 
 * Errored, and Cancelled.
 * 
...
public abstract class Task<R> implements TaskEventHandler, TaskListener<R>, Runnable 
{
	...
	/**
	 * The state of a finished task that has errored.
	 */
	public static final int Errored 	= State.create(6);
	
	/**
	 * The state of a finished task that has been canceled.
	 */
	public static final int Cancelled 	= State.create(7);
	...
	// The error (if any) that occured when the task was executed.
	...
	public void run() 
	{
		...
		synchronized (state) {
			if (state.has(TimedOut | Cancelled)) {
				return;
			}
			state.set(Running);
		}
	 ...
	 * immediately notify the listener if the task can be cancelled.
	 * 
	 * @return
	 * 		True if the task has been cancelled, otherwise false.
	 */
	public boolean cancel() 
	{
		boolean cancellable = state.cas(Waiting, Finished | Cancelled);
		if (cancellable) {
